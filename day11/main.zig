...
fn getGraph(input: []const u8, allocator: std.mem.Allocator) !std.StringHashMap(std.ArrayList([]const u8)) {
    ...
    while (lines.next()) |line| {
        ...
        const deviceOpt = segments.next();
        const restOpt = segments.next();
        std.debug.assert(deviceOpt != null and restOpt != null);
        const device = deviceOpt.?;
        var connectionString = std.mem.tokenizeScalar(u8, restOpt.?, ' ');
